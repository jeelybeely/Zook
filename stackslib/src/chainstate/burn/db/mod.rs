...
use zook_common::util::hash::{hex_bytes, Hash160, Sha512Trunc256Sum};
use zook_common::util::secp256k1::MessageSignature;
use zook_common::util::vrf::*;
...
use crate::chainstate::zbtcz::StacksPublicKey;
...
impl FromColumn<ZBTCZAddress> for ZBTCZAddress {
...
impl FromColumn<BitcoinzAddress> for BitcoinzAddress {
    fn from_column<'a>(row: &'a Row, column_name: &str) -> Result<Self, db_error> {
        let address_str: String = row.get_unwrap(column_name);
        match Self::from_string(&address_str) {
            Some(a) => Ok(a),
