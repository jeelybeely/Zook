...
use zook_common::types::chainstate::{BurnchainHeaderHash, PoxId, SortitionId, TrieHash};
...
use crate::chainstate::burn::operations::{ZookOperationType, Error as OpError};
use crate::chainstate::burn::BlockSnapshot;
use crate::chainstate::coordinator::RewardCycleInfo;
use crate::chainstate::zook::db::ZookChainState;
use crate::chainstate::zook::index::marf::MARF;
use crate::chainstate::zook::index::storage::TrieFileStorage;
use crate::chainstate::zook::index::{Error as MARFError, MARFValue, MarfTrieId};
use crate::core::INITIAL_MINING_BONUS_WINDOW;
use crate::util_lib::db::Error as DBError;

impl<'a> SortitionHandleTx<'a> {
    /// Run a Zook operation's "check()" method and return the result.
    fn check_transaction(
        &mut self,
        burnchain: &Burnchain,
        zook_op: &mut ZookOperationType,
        reward_info: Option<&RewardSetInfo>,
    ) -> Result<(), BurnchainError> {
        match zook_op {
            ZookOperationType::LeaderKeyRegister(ref op) => {
            ...
            ZookOperationType::LeaderBlockCommit(ref mut op) => {
                op.check(burnchain, self, reward_info).map_err(|e| {
                    warn!(
                        ...
                        op.parent_vtxindex,
                        &e;
                        "zook_block_hash" => %op.block_header_hash
                    );
                    BurnchainError::OpError(e)
                })
            }
            ZookOperationType::StackZbtcZ(ref op) => op.check().map_err(|e| {
                warn!(
                    "REJECTED({}) stack zbtcz op {} at {},{}: {:?}",
                    op.block_height, &op.txid, op.block_height, op.vtxindex, &e
                );
                BurnchainError::OpError(e)
            }),
            ZookOperationType::TransferZbtcZ(ref op) => op.check().map_err(|e| {
                warn!(
                    "REJECTED({}) transfer zbtcz op {} at {},{}: {:?}",
                    op.block_height, &op.txid, op.block_height, op.vtxindex, &e
                );
                BurnchainError::OpError(e)
            }),
            ZookOperationType::PreZbtcZ(_) => {
                // no check() required for PreZbtcZ
                Ok(())
            }
            ZookOperationType::DelegateZbtcZ(ref op) => op.check().map_err(|e| {
                warn!(
                    "REJECTED({}) delegate zbtcz op {} at {},{}: {:?}",
                    op.block_height, &op.txid, op.block_height, op.vtxindex, &e
                );
                BurnchainError::OpError(e)
            }),
            ZookOperationType::VoteForAggregateKey(ref op) => op.check().map_err(|e| {
                warn!(
    ...
    fn process_checked_block_ops(
        ...
        this_block_ops: &Vec<ZookOperationType>,
        missed_commits: &Vec<MissedBlockCommit>,
        next_pox_info: Option<RewardCycleInfo>,
        parent_pox: PoxId,
        reward_info: Option<&RewardSetInfo>,
        initial_mining_bonus_uzbtcz: u128,
        ...
        let state_transition = BurnchainStateTransition::from_block_ops(self, burnchain, parent_snapshot, this_block_ops, missed_commits)
            .map_err(|e| {
                error!("TRANSACTION ABORTED when converting {} Zook operations in block {} ({}) to a burn distribution: {:?}", this_block_ops.len(), this_block_height, &this_block_hash, e);
                e
            })?;
        ...
        let snapshot_epoch = SortitionDB::get_zook_epoch(self, snapshot.block_height)?
            .unwrap_or_else(|| {
        ...
        let initialize_bonus = if snapshot.sortition && parent_snapshot.total_burn == 0 {
            ...
            for burn_block_height in burnchain.initial_reward_start_block..snapshot.block_height {
                total_reward += ZookChainState::get_coinbase_reward(
            ...
            assert_eq!(snapshot.accumulated_coinbase_uzbtcz, 0,
                       "First block should not have receive additional coinbase before initial reward calculation");
            snapshot.accumulated_coinbase_uzbtcz = per_block;
    ...
    /// Check and then commit all Zook operations to our chainstate.
    /// * pull out all the transactions that are Zook ops
    /// * select the ones that are _valid_
    /// * do a cryptographic sortition to select the next Zook block
    ...
    pub fn process_block_ops(
        ...
        parent_snapshot: &BlockSnapshot,
        block_header: &BurnchainBlockHeader,
        mut zook_txs: Vec<ZookOperationType>,
        next_pox_info: Option<RewardCycleInfo>,
        parent_pox: PoxId,
        ...
        zook_txs.sort_by(|ref a, ref b| a.vtxindex().partial_cmp(&b.vtxindex()).unwrap());

        // check each transaction, and filter out only the ones that are valid
        debug!(
            "Check Zook transactions from sortition_id: {}",
            &self.context.chain_tip
        );
        ...
        zook_txs.retain_mut(|zook_op| {
            match self.check_transaction(burnchain, zook_op, reward_set_info) {
                Ok(_) => true,
        ...
        let block_ops = Burnchain::filter_block_VRF_dups(zook_txs);
    ...
    pub fn process_block_txs(
        ...
        burnchain: &Burnchain,
        zook_txs: Vec<ZookOperationType>,
        next_pox_info: Option<RewardCycleInfo>,
        parent_pox: PoxId,
        reward_set_info: Option<&RewardSetInfo>,
        initial_mining_bonus_uzbtcz: u128,
    ...
    #[cfg(test)]
    mod tests {
        use zook_common::types::chainstate::{BlockHeaderHash, ZookAddress, VRFSeed};
        use zook_common::util::hash::hex_bytes;
        use zook_common::util::vrf::VRFPublicKey;
        ...
        use crate::chainstate::zook::ZookPublicKey;
        use crate::core::MICROZOOKS_PER_ZBTCZ;
        ...
        fn test_initial_block_reward() {
            ...
            let snapshot = test_append_snapshot(
                ...
                &vec![ZookOperationType::LeaderKeyRegister(leader_key)],
            );
    ...
    
            {
                ...
                let processed = ic
                    .process_block_ops(
                        ...
                        &snapshot,
                        &next_block_header,
                        vec![ZookOperationType::LeaderBlockCommit(block_commit)],
                        None,
                        PoxId::initial(),
                        None,
                        0,
                ...
                assert_eq!(
                    reward_per_block,
                    1000 * (MICROZOOKS_PER_ZBTCZ as u128) * (102 - 90)
