...
// Copyright (C) 2020-2024 ZBTCZ Open Internet Foundation
//
...
use zbtcz_common::deps_common::bitcoinz::blockdata::script::Builder;
use zbtcz_common::types::chainstate::{BurnchainHeaderHash, ZbtczAddress};
use zbtcz_common::types::ZbtczPublicKeyBuffer;
use zbtcz_common::util::secp256k1::Secp256k1PublicKey;

use crate::burnchains::bitcoinz::bits::parse_script;
use crate::burnchains::bitcoinz::{BitcoinzTxInput, BitcoinzTxInputStructured};
...
use crate::chainstate::burn::operations::{
    ...
    ZbtczOperationType, Error as op_error, PreZbtczOp, VoteForAggregateKeyOp,
};
...
struct ParsedData {
    signer_index: u16,
    aggregate_key: ZbtczPublicKeyBuffer,
    round: u32,
    reward_cycle: u64,
}
...
impl VoteForAggregateKeyOp {
    ...
    fn parse_data(data: &Vec<u8>) -> Option<ParsedData> {
        ...
        */
        ...
        let aggregate_key = ZbtczPublicKeyBuffer::from(&data[2..35]);
    ...
    pub fn get_sender_txid(tx: &BurnchainTransaction) -> Result<&Txid, op_error> {
        match tx.get_input_tx_ref(0) {
            Some((ref txid, vout)) => {
                if *vout != 1 {
                    warn!("Invalid tx: VoteForAggregateKey must spend the second output of the PreZbtczOp");
    ...
    pub fn get_sender_pubkey(tx: &BurnchainTransaction) -> Result<Secp256k1PublicKey, op_error> {
        match tx {
            BurnchainTransaction::Bitcoinz(ref btcz) => match btcz.inputs.get(0) {
                Some(BitcoinzTxInput::Raw(input)) => {
                    let script_sig = Builder::from(input.scriptSig.clone()).into_script();
                    let structured_input = BitcoinzTxInputStructured::from_bitcoinz_p2pkh_script_sig(
                ...
                Some(BitcoinzTxInput::Structured(input)) => {
    ...
    /// `Secp256k1PublicKey` from `zbtcz_common`
...
impl ZbtczMessageCodec for VoteForAggregateKeyOp {
...
mod tests {
    use zbtcz_common::deps_common::bitcoinz::blockdata::script::Builder;
    use zbtcz_common::types;
    use zbtcz_common::types::chainstate::{BurnchainHeaderHash, ZbtczAddress};
    use zbtcz_common::types::{Address, ZbtczPublicKeyBuffer};
    use zbtcz_common::util::hash::*;
    use zbtcz_common::util::secp256k1::Secp256k1PublicKey;

    use crate::burnchains::bitcoinz::address::{
    ...
    use crate::burnchains::bitcoinz::{
        ...
        BitcoinzTxInputRaw, BitcoinzTxInputStructured, BitcoinzTxOutput,
    };
    ...
    use crate::chainstate::burn::Opcodes;
    use crate::chainstate::zbtcz::address::{PoxAddress, ZbtczAddressExtensions};

    #[test]
    fn test_parse_vote_tx_signer_key() {
        let aggregate_key = ZbtczPublicKeyBuffer([0x01; 33]);
        let signer_key = ZbtczPublicKeyBuffer([0x02; 33]);
        ...
        let tx = BitcoinzTransaction {
            ...
            outputs: vec![BitcoinzTxOutput {
                units: 10,
                address: BitcoinzAddress::Legacy(LegacyBitcoinzAddress {
        ...
        let vote_op = VoteForAggregateKeyOp::parse_from_tx(
            ...
            &BurnchainTransaction::Bitcoinz(tx),
            &sender,
        )
    ...
    fn test_raw_input_signer_key() {
        ...
        let tx = BitcoinzTransaction {
            ...
            data: vec![1; 47],
            data_amt: 0,
            inputs: vec![BitcoinzTxInput::Raw(BitcoinzTxInputRaw {
    ...
    fn test_key_validation() {
        let sender_addr = "ZB2QKZ4FKHAH1NQKYKYAYZPY440FEPK7GZ1R5HBP2";
        let sender = ZbtczAddress::from_string(sender_addr).unwrap();
        let op = VoteForAggregateKeyOp {
            sender,
            reward_cycle: 10,
            round: 1,
            signer_index: 12,
            signer_key: ZbtczPublicKeyBuffer([0x00; 33]),
            aggregate_key: ZbtczPublicKeyBuffer([0x00; 33]),
            txid: Txid([10u8; 32]),
            vtxindex: 10,
            block_height: 10,
