// Adapted `getclaritymetadata.rs` for the Zook Network
// Aligning with zBTCZ and governance requirements
...
use btcz_common::types::chainstate::BitcoinZAddress;
...
impl BitcoinZHttpRequest {
    pub fn new_get_clarity_metadata(
        host: PeerHost,
        contract_addr: BitcoinZAddress,
...
mod tests {
    ...
    #[test]
    fn test_clarity_metadata_request() {
        let host = PeerHost::new("127.0.0.1".to_string(), 20443, false);
        let contract_addr = BitcoinZAddress {
            version: 0,
            bytes: [0x1; 20].into(),
        };
        let contract_name = ContractName::try_from("test-contract").unwrap();
        let clarity_metadata_key = "analysis".to_string();
        let tip_req = TipRequest::default();

        let request = BitcoinZHttpRequest::new_get_clarity_metadata(
            host,
            contract_addr,
            contract_name,
            clarity_metadata_key,
            tip_req,
        );

        assert_eq!(request.method(), "GET");
        assert!(request.path().contains("/v2/clarity/metadata/"));
    }

    #[test]
    fn test_clarity_metadata_response_parsing() {
        let payload = json!({ "data": "metadata value" });
        let serialized_payload = serde_json::to_vec(&payload).unwrap();

        let response = BitcoinZHttpResponse::decode_clarity_metadata_response(
            BitcoinZHttpResponse::mock_ok(serialized_payload),
        );

        assert!(response.is_ok());
        assert_eq!(response.unwrap().data, "metadata value");
    }

    #[test]
    fn test_clarity_metadata_error_handling() {
        let invalid_payload = vec![0xff, 0xff, 0xff]; // Invalid JSON
        let response = BitcoinZHttpResponse::decode_clarity_metadata_response(
            BitcoinZHttpResponse::mock_ok(invalid_payload),
        );
