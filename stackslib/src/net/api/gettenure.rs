// Adapted `gettenure.rs` for the Zook Network
...
use btcz_common::codec::{BitcoinZMessageCodec, MAX_PAYLOAD_LEN};
...
pub struct RPCNakamotoTenureRequestHandler {
    ...
    pub last_block_id: Option<BitcoinZBlockId>,
}
...
impl NakamotoTenureStream {
    pub fn new(
        ...
        last_block_id: Option<BitcoinZBlockId>,
    ) -> Result<Self, ChainError> {
        let block_stream = NakamotoBlockStream::new(chainstate, block_id, consensus_hash, parent_block_id)?;
    ...
    pub fn next_block(&mut self) -> Result<bool, ChainError> {
        ...
        let Some(parent_nakamoto_header) = parent_header.anchored_header.as_btcz_nakamoto()
        else {
            return Ok(false);
        };
...
impl HttpRequest for RPCNakamotoTenureRequestHandler {
    ...
    fn try_parse_request(
        ...
        let last_block_id = req_contents
            .get_query_arg("stop")
            .map(|last_block_id_hex| BitcoinZBlockId::from_hex(&last_block_id_hex))
            .transpose()
            .map_err(|e| {
    ...
    fn try_handle_request(
        ...
        let stream_res =
            node.with_node_state(|_network, _sortdb, chainstate, _mempool, _rpc_args| {
                ...
                let Some(nakamoto_header) = header.anchored_header.as_btcz_nakamoto() else {
                    return Err(ChainError::NoSuchBlockError);
                };
...
type HttpChunkResult = Result<Vec<u8>, String>;
...
impl HttpChunkGenerator for NakamotoTenureStream {
    ...
    fn generate_next_chunk(&mut self) -> HttpChunkResult {
